...
class DeepNeuralNetwork:
    ...
    def cost(self, Y, A):
        """
        Computes the cost of the model using cross-entropy loss.

        Args:
            Y (numpy.ndarray): Correct labels with shape (classes, m).
            A (numpy.ndarray): Activated output of the neuron with shape (classes, m).
    ...
    def evaluate(self, X, Y):
        ...
        cost = self.cost(Y, A)
        predictions = np.argmax(A, axis=0)  # Class with the highest probability
