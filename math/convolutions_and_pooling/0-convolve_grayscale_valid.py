...
def test_model(network, data, labels, verbose=True):
    """
    Tests a neural network on given data and labels.

    Parameters:
    - network: The network model to test
    - data: The input data to test the model with
    - labels: The correct one-hot labels of data
    - verbose: Boolean to determine if output should be printed during testing

    Returns:
    - Tuple containing the loss and accuracy of the model with the testing data
    """
...
def predict(network, data, verbose=False):
    ...
    - verbose: Boolean to determine if output should be printed during the prediction process
...
def convolve_grayscale_valid(images, kernel):
    ...
    Parameters:
    - images: Numpy array of shape (m, h, w) containing multiple grayscale images
    - kernel: Numpy array of shape (kh, kw) containing the kernel for the convolution
    ...
    for i in range(new_h):
        for j in range(new_w):
            convolved[:, i, j] = np.sum(images[:, i:i+kh, j:j+kw] * kernel, axis=(1, 2))
